...
import type { CatalogCategory, CatalogProduct, KioskCartItem, OrderMode } from '../api';
...
export function productHasModifiers(product: CatalogProduct, modifiersEnabled: boolean): boolean {
  return Boolean(
    modifiersEnabled &&
...
// ── Brand bar logo (compact header above category tabs) ───────────────────────

function BrandBarLogo({ logoUrl, name }: { logoUrl?: string; name: string }) {
  const [failed, setFailed] = useState(false);
  if (logoUrl && !failed) {
    return (
      <img
        className="menu-brand-bar-logo"
        src={logoUrl}
        alt={name}
        onError={() => setFailed(true)}
      />
  ...
  return (
    <div className="menu-brand-bar-initial">
...
export function MenuScreen({
  ...
  function handleProductPress(product: CatalogProduct) {
    const hasModifiers = productHasModifiers(product, modifiersEnabled);
  ...
  const modeBtnLabel = orderMode === 'eatIn' ? 'Comer aquí' : 'Para llevar';
    ...
    return (
      <div className="menu-screen">
        <header className="principal-banner">
          ...
          <button className="mode-badge" onClick={onChangeMode} type="button">
            {modeBtnLabel}
          </button>
        </header>
  ...
  return (
    <div className="menu-screen">
      {/* Compact brand bar: logo + restaurant name */}
      <div className="menu-brand-bar">
        <BrandBarLogo logoUrl={logoUrl} name={restaurantName} />
        {restaurantName && <span className="menu-brand-bar-name">{restaurantName}</span>}
      </div>
